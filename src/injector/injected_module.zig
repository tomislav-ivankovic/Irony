...
pub const InjectedModule = struct {
    ...
    pub fn inject(process: os.Process, module_path: []const u8) !Self {
        ...
        if (os.Module.getRemote(process, file_name) catch null) |module| {
            ejectModule(module) catch |err| {
                misc.error_context.append("Failed eject already loaded module: {s}", .{file_name});
                return err;
            };
        }
        try injectModule(process, module_path);
        const module = os.Module.getRemote(process, file_name) catch |err| {
            misc.error_context.append("Failed get remote module: {s}", .{file_name});
            return err;
        };
        return .{
            .module = module,
    ...
    fn injectModule(process: os.Process, module_path: []const u8) !void {
        ...
        const size = std.unicode.utf8ToUtf16Le(&buffer, module_path) catch |err| {
            misc.error_context.new("Failed to convert UTF8 string \"{s}\" to UTF16-LE.", .{module_path});
            return err;
        };
        const utf16_module_path = buffer[0..size :0];
        const kernel_module = os.Module.getLocal("kernel32.dll") catch |err| {
            misc.error_context.append("Failed to get local kernel module: kernel32.dll", .{});
            return err;
        };
        const load_library_address = kernel_module.getProcedureAddress("LoadLibraryW") catch |err| {
            misc.error_context.append("Failed to get the address of procedure: LoadLibraryW", .{});
            return err;
        };
        const remote_string = os.RemoteSlice(u16, 0).create(process, utf16_module_path) catch |err| {
            misc.error_context.append("Failed to create remote slice containing value: {s}", .{module_path});
            return err;
        };
        defer remote_string.destroy() catch |err| {
            misc.error_context.append("Failed to create destroy remote slice.", .{});
        ...
        const remote_thread = os.RemoteThread.spawn(
            ...
            misc.error_context.append("Failed to spawn remote thread: LoadLibraryW", .{});
            return err;
        };
        defer remote_thread.clean() catch |err| {
            misc.error_context.append("Failed to clean remote thread: LoadLibraryW", .{});
            misc.error_context.logError(err);
        };
        const module_handle_part = remote_thread.join() catch |err| {
            misc.error_context.append("Failed to join remote thread: LoadLibraryW", .{});
            return err;
        };
        if (module_handle_part == 0) {
            misc.error_context.new("Remote LoadLibraryW returned 0.", .{});
    ...
    fn ejectModule(module: os.Module) !void {
        ...
        const free_library_address = kernel_module.getProcedureAddress("FreeLibrary") catch |err| {
            misc.error_context.append("Failed to get the address of procedure: FreeLibrary", .{});
            return err;
        };
        const remote_thread = os.RemoteThread.spawn(
            &module.process,
            ...
            misc.error_context.append("Failed to spawn remote thread: FreeLibrary", .{});
            return err;
        };
        defer remote_thread.clean() catch |err| {
            misc.error_context.append("Failed to clean remote thread: FreeLibrary", .{});
            misc.error_context.logError(err);
        };
        const return_code = remote_thread.join() catch |err| {
            misc.error_context.append("Failed to join remote thread: FreeLibrary", .{});
            return err;
        };
        if (return_code == 0) {
            misc.error_context.new("Remote FreeLibrary returned 0.", .{});
